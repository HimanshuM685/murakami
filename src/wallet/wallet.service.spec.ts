...
describe('WalletService', () => {
  ...
  describe('transferAlgoToAddress()', () => {
    ...
    const dummySignedTx = new Uint8Array([4, 5, 6]);

    beforeEach(() => {
      chainServiceMock.getSuggestedParams.mockResolvedValueOnce(suggestedParams);
      chainServiceMock.craftPaymentTx.mockResolvedValueOnce(dummyTx);
      chainServiceMock.submitTransaction.mockResolvedValueOnce({ txid: 'pay_tx_id' } as any);
    ...
    it('transferAlgoToAddress() -- as manager with lease and note', async () => {
      vaultServiceMock.getManagerPublicKey.mockResolvedValueOnce(managerPubKey);
      ...
      expect(walletService.signTxAsManager).toHaveBeenCalledWith(dummyTx, vaultToken);
      expect(chainServiceMock.submitTransaction).toHaveBeenCalledWith(dummySignedTx);
      expect(result).toBe('pay_tx_id');
    });
    ...
    it('transferAlgoToAddress() -- as user with lease and note', async () => {
      ...
      expect(walletService.signTxAsUser).toHaveBeenCalledWith(userId, dummyTx, vaultToken);
      expect(chainServiceMock.submitTransaction).toHaveBeenCalledWith(dummySignedTx);
      expect(result).toBe('pay_tx_id');
    });
  });
